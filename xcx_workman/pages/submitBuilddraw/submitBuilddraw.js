// pages/submitBuilddraw/submitBuilddraw.js
const app = getApp()
...
Page({
  ...
  data: {
    ...
    classification: [],
    // 遮罩标题
    showModaltitle: ["建筑层数", "建筑风格", "结构形式", "主体造价", "占地面积"],
    showModaltype: 0,
    // 按钮禁用
    btnFlag: false,
    // 协议
  ...
  getBuildDraw:function(id){
    var that = this
    qingqiu.get("buildQueryById",{id:id},function(res){
      console.log(res)
      if(res.result != null){
        var m = res.result
        var piclist = []
        ...
        that.setData({
          id: m.id,
          layerid: m.layerId,
          layertext:m.layerName,
          buildstyleId: m.buildStyleId,
          buildstyle:m.buildStyleName,
          structureId: m.structureId,
          structure:m.structureName,
          manufacturId: m.manufacturId,
          manufactur:m.manufacturName,
          buildareaId: m.buildAreaId,
          buildarea:m.buildAreaName,
          builddraw: m.title,
          buildphone: m.bphone,
          buildcontent: m.bcontent,
          tupianlists:piclist,
          num:piclist.length
        })
      }      
  ...
  submit: function () {
    ...
    var s = qingqiu.yanzheng(para.layerid + ",选择建筑层数|" + para.buildstyleId + ",选择建筑风格|" + para.structureId + ",选择结构形式|" + para.manufacturId + ",选择主体造价|" + para.buildareaId + ",选择占地面积|" + para.builddraw + ",输入图纸标题|" + para.buildphone + ",输入联系电话")
    if (s != 0) {
      wx.showToast({
        title: s,
        icon: "none"
    ...
    var pic = ""
    for (let obj of para.tupianlists) {
      pic += obj + ','
    }
    pic = pic.substring(0, pic.lastIndexOf(',')); // 删除最后一个逗号
    var data = {
      wxUserId: 1558,
      layerId: para.layerid,
      buildStyleId: para.buildstyleId,
      structureId: para.structureId,
      manufacturId: para.manufacturId,
      buildAreaId: para.buildareaId,
      title: para.builddraw,
      bPhone: para.buildphone,
      bContent: para.buildcontent,
      pic: pic
    }
    console.log(data)
    // 判断id是否存在，不存在添加，存在则修改
    if (para.id == 0) {
      qingqiu.get("buildAdd", data, function (res) {
        console.log(res)
        if (res.result == 0) {
          wx.showToast({
            title: '发布成功',
      ...
      data.id = para.id
      qingqiu.get("buildEdit", data, function (res) {
        console.log(res)
        if(res.result == 0){
          wx.showToast({
            title: '修改成功',
    ...
    wx.chooseImage({
      ...
      success: function (res) {
        ...
        for (let i = 0; i < tempFilePaths.length; i++) {
          if (!/\.(jpg|jpeg|png|JPG|PNG|ico)$/.test(tempFilePaths[i])) {
            wx.showToast({
              title: '请上传静态图片',
              icon: 'none'
            })
                ...
                wx.uploadFile({
                  ...
                  success(res) {
                    ...
                    that.setData({
                      ...
                      num: that.data.tupianlists.length
                    })
                  }
                })
                index2 += 1
   ...
   * @param {图纸详情} e 
   */
  buildcontentinput: function (e) {
    this.setData({
      buildcontent: e.detail.value
    })
  },
   ...
   * @param {联系电话} e 
   */
  buildphoneinput: function (e) {
    this.setData({
      buildphone: e.detail.value
    })
  },
   ...
   * @param {图纸标题} e 
   */
  builddrawinput: function (e) {
    console.log('图纸标题携带值', e.detail.value)
    this.setData({
      builddraw: e.detail.value
    })
  },
  /**
   * 图纸标题过滤
  ...
  changeclass: function (e) {
    ...
    switch (this.data.showModaltype) {
      case "0":
        this.setData({
          layerid: id,
          layertext: name
